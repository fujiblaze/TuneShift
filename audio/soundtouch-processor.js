...
function readFrameSample(src, srcOffset, numFrames, frameIndex, channel, state) {
	if (frameIndex < 0) {
		const history = channel === 0 ? state.historyL : state.historyR;
		const historyIndex = (state.historyFrames ?? 0) + frameIndex;
		if (history && historyIndex >= 0) return history[historyIndex];
...
function lanczosWeight(distance, support, cutoff) {
	if (Math.abs(distance) >= support) return 0;
	return cutoff * normalizedSinc(cutoff * distance) * normalizedSinc(distance / support);
}
var lanczosKernel = (src, srcOffset, numFrames, position, channel, state) => {
	...
	const cutoff = Math.min(1, 1 / Math.max(1e-6, Math.abs(kernelState.rate ?? 1)));
	const support = radius / cutoff;
	const normalize = Boolean(kernelState.params.normalize);
	const center = Math.floor(position);
	const supportFrames = Math.ceil(support);
	const start = center - (supportFrames - 1);
	const end = center + supportFrames;
	let numerator = 0;
	let denominator = 0;
	for (let sampleIndex = start; sampleIndex <= end; sampleIndex += 1) {
		const weight = lanczosWeight(position - sampleIndex, support, cutoff);
...
lanczosKernel.createState = () => ({
	...
	prevSampleR: 0,
	rate: 1,
	historyL: new Float32Array(128),
	historyR: new Float32Array(128),
	historyFrames: 0,
...
var RateTransposer = class RateTransposer extends AbstractSamplePipe {
	...
	*/
	reset() {
		...
		if (this.kernelState?.historyL) {
			this.kernelState.historyL.fill(0);
			this.kernelState.historyR.fill(0);
			this.kernelState.historyFrames = 0;
		}
	}
	/**
	...
	*/
	process() {
		...
		if (this._consumedFrames > 0) this.inputAdapter.receive(this._consumedFrames);
	...
	*/
	transpose(numFrames = 0) {
		...
		const radius = Number(this.interpolationStrategyParams.zeroCrossings ?? 4);
		const lookaheadFrames = Math.ceil(radius * Math.max(1, Math.abs(this._rate)));
		this._consumedFrames = Math.max(0, numFrames - lookaheadFrames);
		if (this._consumedFrames === 0) return 0;
		return this.transposePluginKernel(numFrames, this._consumedFrames);
	}
	/**
	...
	*/
	transposePluginKernel(numFrames, consumedFrames = numFrames) {
		const src = this.inputScratch;
		...
		if (stateRecord !== void 0) {
			...
			stateRecord.rate = this._rate;
		}
		let i = 0;
		let position = this.fractionalPosition;
		const maxPosition = consumedFrames - 1;
		...
		this.fractionalPosition = position - consumedFrames;
		this.previousLeftSample = src[srcOffset + 2 * consumedFrames - 2];
		this.previousRightSample = src[srcOffset + 2 * consumedFrames - 1];
		if (stateRecord !== void 0) {
			this.updateKernelHistory(stateRecord, src, srcOffset, consumedFrames);
			stateRecord.prevSampleL = this.previousLeftSample;
			stateRecord.prevSampleR = this.previousRightSample;
		}
		return i;
	}
	updateKernelHistory(state, src, srcOffset, consumedFrames) {
		if (!state.historyL || !state.historyR || consumedFrames <= 0) return;
		const capacity = state.historyL.length;
		const incoming = Math.min(consumedFrames, capacity);
		const retained = Math.min(state.historyFrames ?? 0, capacity - incoming);
		if (retained > 0) {
			state.historyL.copyWithin(0, (state.historyFrames ?? 0) - retained, state.historyFrames ?? 0);
			state.historyR.copyWithin(0, (state.historyFrames ?? 0) - retained, state.historyFrames ?? 0);
		}
		const sourceStart = consumedFrames - incoming;
		for (let index = 0; index < incoming; index++) {
			state.historyL[retained + index] = src[srcOffset + 2 * (sourceStart + index)];
			state.historyR[retained + index] = src[srcOffset + 2 * (sourceStart + index) + 1];
		}
		state.historyFrames = retained + incoming;
	}
	/**
...
var SoundTouch = class SoundTouch {
	...
	*
	* @remarks
	* `_rate` is set to `virtualPitch`; `_tempo` to `1 / virtualPitch`. The Stretch
	* Stretch stage always feeds the Transposer so crossing unity does not tear down and
	* refill a differently ordered pipeline in the middle of playback.
	*/
